 ...
 */
namespace codicastudio\UserVerification\Mail;
...
class VerificationTokenGenerated extends Mailable
{
    ...
    public function build()
    {
        ...
        $this->subject(is_null($this->subject)
            ? trans('user-verification::user-verification.verification_email_subject')
            : $this->subject);

        if (config('user-verification.email.type') == 'markdown') {
            is_null($view = config('user-verification.email.view'))
                ? $this->markdown('user-verification::email-markdown')
                : $this->markdown($view);
        } else {
            is_null($view = config('user-verification.email.view'))
                ? $this->view('user-verification::email')
                : $this->view($view);
        }
